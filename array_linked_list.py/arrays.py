...
def array_read(array, index):
    ...
    if index >= array.count:
        print(f'Error! index ' + str(index) + f'out of range')
        return None
...
def array_remove(array, value):
    ...
    for i in range(array.count):
        if removed:
            array.value[i-1]
        elif array.value[i] == value:
            # very first occurance of this value
            removed = True
    if removed:
        array.count -= 1
        array.value[array.count] = None
    else:
...
def array_pop(array, index):
    # Throw an error if array is out of the current count
    if index > array.count:
        print("Error, element index" + str(index) + "is out of range")
        return None
    # Your code here
